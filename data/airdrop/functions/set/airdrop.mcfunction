execute if score @s air_set matches 4 run scoreboard players operation @e[type=marker,tag=yes,limit=1] air_borne >< @e[type=marker,tag=no,limit=1] air_borne
execute if score @s air_set matches 4 run tellraw @a ["§8<書本調整-§b空投§8> §7空投已調整為",{"selector":"@e[type=marker,tag=set,limit=1,scores={air_borne=1}]"}]

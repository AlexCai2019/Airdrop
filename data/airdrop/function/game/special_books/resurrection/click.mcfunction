#檢查身上有這本書
execute unless items entity @s container.* written_book[custom_data~{resurrection: true}] run return run tellraw @s ["<", {"text": "轉生之術", "color": "aqua"}, "> 轉生之術發動失敗…身上沒有書本。"]
...
#失敗 找不到已死亡的隊友
execute unless entity @a[predicate=airdrop:resurrection] run return run tellraw @s ["<", {"text": "轉生之術", "color": "aqua"}, "> 轉生之術發動失敗…沒有已死亡的隊友。"]

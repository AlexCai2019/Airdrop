...
#球體粒子
execute if predicate airdrop:sphere_range if items entity @s weapon warped_fungus_on_a_stick[custom_data~{gadget: "special_block_wand"}] run function airdrop:game/effects/sphere

#四大特殊方塊
execute if block ~ ~-1 ~ melon unless score #ban_melon air_time matches 1.. run return run function airdrop:game/effects/melon/on

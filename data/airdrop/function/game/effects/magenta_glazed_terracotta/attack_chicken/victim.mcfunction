...
execute at @s as @a if score @s air_uuid = #victim_chicken air_uuid_match run tellraw @s {translate: "death.attack.player", with: [{selector: "@e[type=chicken, tag=air_victim, distance=..0.01]"}, {selector: "@a[advancements={airdrop:command/attack_chicken=true}]"}]}

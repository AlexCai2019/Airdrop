...
playsound entity.generic.explode player @a ~ ~ ~ 1.0 1.0 1.0
execute store result score #difficulty air_wand run difficulty

#對最近的敵人發起攻擊
tag @s add air_explode
execute as @p[tag=air_enemy_explode, distance=..5] run function airdrop:game/effects/magenta_glazed_terracotta/explode/victim
#其餘敵人不再有tag
tag @a[tag=air_enemy_explode, distance=..5] remove air_enemy_explode
#自己也爆

...
#刪除下面不是西瓜的倒數用展示實體
execute if entity @s[type=text_display, tag=air_melon_timer] unless block ~ ~-1 ~ melon run return run kill

#隱形追蹤者
execute if entity @s[type=armor_stand, tag=air_detect_invisible] anchored eyes run return run function airdrop:game/detect_invisible/detect

#終界石子彈
execute if entity @s[type=marker, tag=air_end_stone_bullet] run return run function airdrop:game/effects/end_stone/bullet/tick

...
#0、1、2可以有特殊處理來最佳化
execute if score #item_damage air_drop matches 0 run return fail
execute if score #item_damage air_drop matches 1 run return run function airdrop:game/special_arrows/extra/item_damage/index/1
execute if score #item_damage air_drop matches 2 run return run function airdrop:game/special_arrows/extra/item_damage/index/2

#random的max是包含的 所以必須先-1
execute store result storage airdrop:data root.special_arrows.item_damage.range int 1 run scoreboard players remove #item_damage air_drop 1
